...
const checkLanguage = async (language, forceUpdate) => {
  ...
  if (!parsedData) {
    logger.warn(
      `- discarding new version folder ${newDataF} because new data could not be generated`
    );
    try {
      await rimraf(newDataF);
    } catch (e) {
      logger.error(
        `error while removing folder ${newDataF}. Next version number might be affected.`,
        e
      );
    }
    return false;
  ...
  if (!arr_diff.length > 0) {
    ...
    logger.debug(
      `- discarding new version folder ${newDataF}. Everything is the same.`
    );
    try {
      await rimraf(newDataF);
    } catch (e) {
...
const gatherData = async (data, newDataF) => {
  ...
  let patch_notes = await generateData(
    generatePatchNotes,
    "patch_notes",
    data,
    newDataF
