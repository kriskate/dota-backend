...
export const getEnglishPatchNotes = async () => {
  if (!_englishPatchNotes) {
    _englishPatchNotes = await fetcher(
      fetcher_TYPES.TXT,
      "localization_patch_notes",
      "english"
    );
  }
...
export const getRawData = async lg => {
  try {
    return {
      localization_abilities: await fetcher(
        fetcher_TYPES.JSON,
        "localization_abilities",
        lg
      ),
      localization_dota: await fetcher(
        fetcher_TYPES.JSON,
        "localization_dota",
        lg
      ),
      localization_hero_lore: await fetcher(
        fetcher_TYPES.HeroLore,
        "localization_hero_lore",
        lg
      ),
      localization_patch_notes: await fetcher(
        fetcher_TYPES.TXT,
        "localization_patch_notes",
        lg
      ),
...
export const checkSize = async (fileName, oldDataF, newDataF) => {
  try {
    if (!fs.existsSync(`${oldDataF}/${fileName}.json`))
    ...
    logger.debug(
      `comparing ${fileName} size (${newStat.size}) to old file size (${
        oldStat.size
      }); ${
    ...
    logger.warn(
      `error while retrieving stats for old or new data; old: ${oldDataF}, new: ${newDataF}`,
      e
    );

    // returning false because we don't want the current file to be overwritten
    return false;
  }
};
